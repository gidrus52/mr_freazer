import { Cookie, Public, UserAgent, Roles } from '@common/decorators';
...
import { RolesGuard } from './guargs/role.guard';
...
export class AuthController {
    ...
    async registerAdmin(@Body() dto: RegisterDto) {
        ...
        if (!user) {
            throw new BadRequestException(
                `Не получается зарегистрировать администратора с данными ${JSON.stringify(dto)}`,
            );
        }
        return new UserResponse(user);
    }
    ...
    @Public()
    async registerFirstAdmin(@Body() dto: RegisterDto) {
        const user = await this.authService.registerFirstAdmin(dto);
        if (!user) {
            throw new BadRequestException(
                `Не получается зарегистрировать первого администратора с данными ${JSON.stringify(dto)}`,
            );
        }
        return new UserResponse(user);
    }
